...
//  Helpers
//--------------------------------------------------------------------------------------------------------//
Scene.prototype.loadShader = function (
  shaderName,
  shaderType,
  shaderTypeString
) {
...
Scene.prototype.initialiseShaders = function () {
  ...
  this.shaderVertex = this.loadShader(
    "vertex-shader",
    this.gl.VERTEX_SHADER,
    "VERTEX"
  );
  this.shaderFragment = this.loadShader(
    "fragment-shader",
  ...
  this.indexPosition = this.gl.getAttribLocation(
    this.shaderProgram,
    "position"
  );
  this.indexTexCoords = this.gl.getAttribLocation(
    this.shaderProgram,
    "texcoords"
  );
...
Scene.prototype.initialiseMatrices = function () {
  this.indexMatrixView = this.gl.getUniformLocation(
    this.shaderProgram,
    "viewMatrix"
  );
  this.indexMatrixModel = this.gl.getUniformLocation(
    this.shaderProgram,
    "modelMatrix"
  );
  this.indexMatrixProjection = this.gl.getUniformLocation(
    this.shaderProgram,
    "projectionMatrix"
  );
...
Scene.prototype.setViewFrustum = function (near, far, fov) {
  matrixHelper.matrix4.makeProjection(
    this.matrixProjection,
    near,
    far,
    fov,
...
Scene.prototype.findNode = function (nodeName) {
  ...
  while (stack.length != 0) {
    ...
    if (node.name.localeCompare(nodeName) == 0) return node;
